...
(function (global) {
  ...
  // Mesma automação do Modo Vendedor (ver vendedorMode.js) reaproveitada
  // aqui no PDV de mesa — os dois são o mesmo contexto presencial da usuária
  // (decisão explícita: Vitrine substitui o WhatsApp; Vender/Modo Vendedor
  // presencial ganham PIX + cartão via Mercado Pago, sem boleto). Helpers
  // duplicados de vendedorMode.js (mesmo padrão já usado no backend entre
  // operations.js/salesCore.js) — cada tela de venda já é independente e sem
  // bundler, então copiar um bloco pequeno e autocontido é mais simples do
  // que criar um módulo compartilhado só pra isso (2026-08-28).
      ...
      pollTimer = setInterval(function () {
        ...
        App.api.payments.presencialChargeStatus(chargeId).then(function (charge) {
          if (charge.status === 'pago') {
            stopPolling();
            onConfirmed({ methodId: mpMethod.id, amount: amount, mpPaymentId: charge.mpPaymentId });
            modalRef.close();
  ...
  function renderPaymentColumn(col) {
    ...
    // "PIX (Mercado Pago)"/"Cartão (Mercado Pago)" nunca ficam no <select>
    // manual — só são usadas por baixo dos panos pelos botões de cobrança
    // automática abaixo, pra nunca deixar marcar "pago" sem o Mercado Pago
    // ter confirmado de verdade (2026-08-28).
    var manualMethods = state.paymentMethods.filter(function (m) { return (m.name || '').indexOf('Mercado Pago') === -1; });
    var methodSelect = App.ui.el('select', { id: 'pdv-method' }, manualMethods.map(function (m) {
    ...
    if (App.api && App.api.enabled) {
      if (pixMpMethod) {
        mpButtons.push(App.ui.el('button', {
          ...
          onclick: function () {
            openMpChargeModal('pix', pixMpMethod, remaining > 0 ? remaining : totals.total, function (payment) {
              state.payments.push({ id: App.core.uuid(), methodId: payment.methodId, amount: payment.amount, mpPaymentId: payment.mpPaymentId });
      ...
      if (cardMpMethod) {
        mpButtons.push(App.ui.el('button', {
          class: 'btn btn-secondary btn-sm',
          onclick: function () {
            openMpChargeModal('cartao', cardMpMethod, remaining > 0 ? remaining : totals.total, function (payment) {
              state.payments.push({ id: App.core.uuid(), methodId: payment.methodId, amount: payment.amount, mpPaymentId: payment.mpPaymentId });
              renderPaymentColumn(col);
            });
          }
    ...
    col.appendChild(App.ui.el('div', { class: 'card' }, [
      App.ui.el('div', { class: 'card-body' }, [
        ...
        mpButtons.length ? App.ui.el('div', { class: 'vendor-mp-buttons', style: 'margin-top:8px;' }, mpButtons) : null,
        paymentRows,
  ...
  function finalizeSale() {
    ...
    var params = {
      ...
      payments: state.payments.map(function (p) { return { methodId: p.methodId, amount: p.amount, mpPaymentId: p.mpPaymentId || null }; }),
