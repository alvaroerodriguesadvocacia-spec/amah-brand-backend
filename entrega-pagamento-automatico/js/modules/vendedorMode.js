...
(function (global) {
  ...
  // Pagamento automático (Mercado Pago), só presencial: PIX (QR Code
  // devolvido na hora, mostrado na tela da vendedora) e cartão (link/QR pro
  // celular do PRÓPRIO cliente escanear — decisão explícita: sem maquininha,
  // sem boleto presencial). As formas "PIX (Mercado Pago)"/"Cartão (Mercado
  // Pago)" cadastradas em Configurações nunca aparecem no <select> manual —
  // só são usadas por baixo dos panos por este fluxo, pra manter a taxa e o
  // rastreio (mpPaymentId) corretos no relatório (2026-08-28).
  function findMpMethod(methods, label) {
    return methods.filter(function (m) { return (m.name || '').indexOf(label) === 0; })[0] || null;
  }

  function openMpChargeModal(methodType, mpMethod, suggestedAmount, onConfirmed) {
    var amountInput = App.ui.el('input', { type: 'number', step: '0.01', min: '0.01', value: suggestedAmount > 0 ? suggestedAmount.toFixed(2) : '' });
    var errorBox = App.ui.el('div', { class: 'modal-alert hidden' });
    var stage = App.ui.el('div', { class: 'vendor-mp-stage' });
    var pollTimer = null, pollCount = 0;
    var MAX_POLLS = 100; // ~5 minutos (100 x 3s)

    var body = App.ui.el('div', {}, [
      errorBox,
      App.ui.el('div', { class: 'form-field' }, [
        App.ui.el('label', {}, ['Valor a cobrar (R$)']),
        amountInput
      ]),
      stage
    ]);
    ...
    var modalRef = App.ui.openModal({
      title: methodType === 'pix' ? 'Cobrar PIX (Mercado Pago)' : 'Cobrar no cartão (Mercado Pago)',
      bodyNode: body,
      closeOnBackdrop: false,
      onClose: stopPolling,
      footerButtons: [
        {
          label: 'Gerar cobrança', className: 'btn-primary',
          onClick: function () {
            var amount = round2(Number(amountInput.value));
            ...
            stage.innerHTML = '';
            stage.appendChild(App.ui.el('div', { class: 'vendor-mp-loading' }, ['Gerando cobrança…']));
            App.api.payments.presencialCharge({ amount: amount, methodType: methodType }).then(function (charge) {
              renderChargeStage(charge, amount);
              startPolling(charge.id, amount);
            }).catch(function (err) {
              stage.innerHTML = '';
        ...
        { label: 'Fechar', className: 'btn-secondary', onClick: function (close) { stopPolling(); close(); } }
      ]
    });

    function renderChargeStage(charge, amount) {
      stage.innerHTML = '';
      if (methodType === 'pix') {
        if (charge.qrCodeBase64) {
          stage.appendChild(App.ui.el('img', { src: 'data:image/png;base64,' + charge.qrCodeBase64, alt: 'QR Code PIX', class: 'vendor-mp-qr' }));
        }
        if (charge.qrCode) {
          var codeBox = App.ui.el('textarea', { class: 'vendor-mp-code', readonly: 'readonly', rows: '3' }, [charge.qrCode]);
          stage.appendChild(codeBox);
          stage.appendChild(App.ui.el('button', {
            type: 'button', class: 'btn btn-secondary btn-sm',
            onclick: function () {
              codeBox.select();
              if (navigator.clipboard && navigator.clipboard.writeText) navigator.clipboard.writeText(charge.qrCode).catch(function () {});
              App.ui.toast('Código PIX copiado.', 'success');
            }
          }, ['Copiar código PIX']));
        }
        stage.appendChild(App.ui.el('p', { class: 'vendor-mp-hint' }, ['Mostre este QR Code pro cliente escanear no app do banco dele — a venda confirma sozinha assim que o pagamento cair.']));
      } else {
        if (charge.initPoint) {
          var qrDataUrl = App.core.labels.qrDataUrl(charge.initPoint);
          if (qrDataUrl) stage.appendChild(App.ui.el('img', { src: qrDataUrl, alt: 'QR Code do link de pagamento', class: 'vendor-mp-qr' }));
          stage.appendChild(App.ui.el('p', { class: 'vendor-mp-hint' }, ['Peça pro cliente escanear este QR Code com a câmera do celular DELE (ou envie o link abaixo) — ele escolhe o cartão e paga direto no aparelho dele.']));
          stage.appendChild(App.ui.el('a', { href: charge.initPoint, target: '_blank', rel: 'noopener', class: 'vendor-mp-link' }, ['Abrir link de pagamento']));
        }
      }
      stage.appendChild(App.ui.el('div', { class: 'vendor-mp-waiting' }, ['⏳ Aguardando o pagamento (' + fmt.money(amount) + ')…']));
    }

    function startPolling(chargeId, amount) {
      pollCount = 0;
      pollTimer = setInterval(function () {
        pollCount++;
        App.api.payments.presencialChargeStatus(chargeId).then(function (charge) {
          if (charge.status === 'pago') {
            stopPolling();
            onConfirmed({ methodId: mpMethod.id, methodName: mpMethod.name, amount: amount, mpPaymentId: charge.mpPaymentId });
            modalRef.close();
            App.ui.toast('Pagamento confirmado! ✅', 'success');
            return;
          }
          if (pollCount >= MAX_POLLS) {
            stopPolling();
            var waitingEl = stage.querySelector('.vendor-mp-waiting');
            if (waitingEl) waitingEl.textContent = '⚠️ Tempo esgotado esperando o pagamento. Se o cliente já pagou, aguarde mais um pouco e reabra; senão, feche e tente de novo.';
          }
        }).catch(function () { /* falha de rede pontual — tenta de novo no próximo tick */ });
      }, 3000);
    }
  }
  ...
  function openPaymentSheet() {
    ...
    var total = cartTotal();
    var payments = []; // { methodId, methodName, amount, mpPaymentId? }
    var pixMpMethod = findMpMethod(state.paymentMethods, 'PIX (Mercado Pago)');
    var cardMpMethod = findMpMethod(state.paymentMethods, 'Cartão (Mercado Pago)');

    // As formas automáticas nunca ficam no <select> manual — só as formas
    // tradicionais (Dinheiro, PIX manual, Débito, Crédito, Boleto etc.).
    var manualMethods = state.paymentMethods.filter(function (m) { return (m.name || '').indexOf('Mercado Pago') === -1; });
    var methodSelect = App.ui.el('select', { id: 'vendor-pay-method' },
      manualMethods.map(function (m) { return App.ui.el('option', { value: m.id }, [m.name]); }));
    ...
    // Cobrança automática (Mercado Pago) — só aparece se o backend estiver
    // configurado e a forma correspondente existir em Configurações. Cada
    // botão abre um modal próprio que gera a cobrança, mostra o QR Code e
    // acrescenta o pagamento à lista sozinho assim que confirmado (nunca
    // manualmente, pra não deixar marcar "pago" sem o Mercado Pago ter
    // confirmado de verdade).
    var mpButtons = [];
    if (App.api && App.api.enabled) {
      if (pixMpMethod) {
        mpButtons.push(App.ui.el('button', {
          type: 'button', class: 'btn btn-secondary',
          onclick: function () {
            openMpChargeModal('pix', pixMpMethod, remaining() > 0 ? remaining() : total, function (payment) {
      ...
      if (cardMpMethod) {
        mpButtons.push(App.ui.el('button', {
          type: 'button', class: 'btn btn-secondary',
          onclick: function () {
            openMpChargeModal('cartao', cardMpMethod, remaining() > 0 ? remaining() : total, function (payment) {
              payments.push(payment);
              refresh();
            });
          }
    ...
    var body = App.ui.el('div', {}, [
      ...
      addBtn,
      mpButtons.length ? App.ui.el('div', { class: 'vendor-mp-buttons' }, mpButtons) : null,
      listWrap,
      remainingLabel
    ]);
    ...
    function doFinalize(paymentList, close) {
      var params = {
        ...
        payments: paymentList.map(function (p) { return { methodId: p.methodId, amount: p.amount, mpPaymentId: p.mpPaymentId || null }; }),
