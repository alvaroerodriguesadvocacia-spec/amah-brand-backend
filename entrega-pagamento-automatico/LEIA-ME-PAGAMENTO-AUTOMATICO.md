...
## É seguro aplicar agora?

Sim. Enquanto o Mercado Pago não estiver configurado (próximo passo abaixo), tudo continua funcionando **exatamente como hoje** — a Vitrine cai de volta pro WhatsApp e os botões de cobrança presencial simplesmente não aparecem. Você pode aplicar este pacote e testar com calma antes de ligar o pagamento automático de verdade.

## Passo 1 — Aplicar os arquivos (GitHub Desktop, como sempre)

Este pacote contém os arquivos que mudaram, já na pasta certa. Copie cada um para o lugar correspondente na sua pasta do projeto (substituindo o arquivo antigo), depois faça commit e push pelo GitHub Desktop, do mesmo jeito de sempre. Arquivos incluídos:

- `backend/src/mercadoPago.js` (novo)
- `backend/src/salesCore.js` (novo)
- `backend/src/routes/payments.js` (novo)
- `backend/src/db.js`
- `backend/src/app.js`
...
*(Existe um segundo site de Vitrine na sua conta do Render, `amah-brand-vitrine-v2` — é uma cópia de diagnóstico de um problema já resolvido em 2026-08-21, não é a que você usa. Se um dia quiser removê-la, é opcional, pelo painel do Render.)*
